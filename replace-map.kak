...
The map used for lookups is specified using the content of the copy register (double quote, ")
Switches:
    ...
    eval %sh{
        ...
        while [ $# -ne 0 ]; do
            ...
            if [ "$arg" = '-map-order' ]; then
                ...
                case "$1" in
                    "'") register="''" ;;
                    *"'"*) printf 'fail "BOB"' # need to early exit here, otherwise the %reg'' call won't work
                           exit 1
                           ;;
                    *) register="$1" ;;
                esac
